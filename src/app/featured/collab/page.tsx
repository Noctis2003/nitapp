...
export default function CollabListPage() {
  ...
  return (
    <div
      className={`flex flex-grow xxs:mt-4 md:mt-0 bg-gray-950 text-white px-4 py-10 ${poppins.className}`}
    >
      <div className="max-w-5xl mx-auto xxs:mt-2 md:mt-0">
          ...
          {loading ? (
            ...
            collabs.map((collab, idx) => (
              <div
                key={idx}
                onClick={() =>
                  router.push(`/featured/collab/${collab.id}`)
                }
                className="border-2 cursor-pointer border-cyan-500 shadow-cyan-500/50 p-4 xxs:w-full md:w-auto h-30  flex flex-grow flex-col justify-between mt-12 from-gray-900 to-gray-80 rounded-3xl shadow-[0_0_20px_#22d3ee22]  hover:bg-gray-900 bg-gradient-to-br transition-all duration-300 ease-in-out "
              >
